...
w = []
# w = [[[0.25, 0.34, 0.4], [-0.2, 0.12, 0.34], [0.3, 0.12, 0.000001], [-0.21, -0.12, 0.2]],[[-0.2, 0.1, -0.1, 0.24,
# 0.45]]] # Fixed weights for developement
x = [0, 0, 1, 1]
y = [0, 1, 0, 1]
t = [0, 0, 0, 1]  # AND
# t = [0, 1, 1, 1]  # OR
# t = [0, 1, 1, 0]  # XOR
hidden_output = 4 * [[0, 0, 0, 0]]  # Initiales Array für die Outputs der Hidden-Schicht
...
def get_neural_output_output(input, i):  # input: Output Neuron 1 + Output Neuron 2 + Output Neuron 3 + Output Neuron 4
    ...
    return 1 / (1 + math.exp(-10 * (
            bias * w[1][i][0] + input[0] * w[1][i][1] + input[1] * w[1][i][2] + input[2] * w[1][i][3] + input[3] *
            w[1][i][4])))
...
def get_neural_output_final(x, y):
    ...
    layer_1 = [get_neural_output_hidden([x, y], 0), get_neural_output_hidden([x, y], 1),
               get_neural_output_hidden([x, y], 2), get_neural_output_hidden([x, y], 3)]
...
def train():
    ...
    for i in range(len(w[0])):  # Für jedes Neuron der Schicht
        ...
        for q in range(num_patterns):
            ...
            hidden_output[i][q] = get_neural_output_hidden(input, i)
    ...
    for i in range(len(w[1])):
        ...
        for q in range(num_patterns):
            input = [hidden_output[0][q], hidden_output[1][q], hidden_output[2][q], hidden_output[3][q]]
...
def main():
    ...
    im = Image.new('RGB', (PNG_WIDTH, PNG_HEIGHT))
